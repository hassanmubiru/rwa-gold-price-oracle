...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log("[1/4] Fetching PAXG price from CoinGecko across CRE nodes…");
  const currentPriceCents = runtime2.runInNodeMode(fetchPaxgPriceCents, consensusMedianAggregation())().result();
  runtime2.log(`[1/4] ✔ Consensus PAXG price: $${(Number(currentPriceCents) / 100).toFixed(2)} USD`);
  runtime2.log("[2/4] Reading previous price from on-chain Storage contract…");
  ...
  const contractResult = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
  ...
  runtime2.log(`[2/4] ✔ Previous on-chain price: $${(Number(previousPriceCents) / 100).toFixed(2)} USD`);
  runtime2.log("[3/4] Computing price change…");
  let priceChangeBps = 0n;
  if (previousPriceCents > 0n) {
    priceChangeBps = (currentPriceCents - previousPriceCents) * 10000n / previousPriceCents;
  }
  runtime2.log(`[3/4] ✔ Price change: ${priceChangeBps} bps (${Number(priceChangeBps) / 100}%)`);
  runtime2.log("[4/4] Writing verified oracle price update on-chain…");
  const txHash = publishOracleUpdate(runtime2, network248.chainSelector.selector, evmConfig, currentPriceCents, priceChangeBps, currentPriceCents);
  const result = {
  ...
  runtime2.log(`[4/4] ✔ Oracle update published — PAXG: $${(Number(currentPriceCents) / 100).toFixed(2)} | Δ: ${priceChangeBps} bps | tx: ${txHash}`);
  return result;
};
var fetchPaxgPriceCents = (nodeRuntime) => {
...
function publishOracleUpdate(runtime2, chainSelector, evmConfig, offchainValue, onchainValue, finalResult) {
  ...
  runtime2.log(`On-chain write succeeded! txHash: ${txHash}`);
  runtime2.log(`Etherscan: https://sepolia.etherscan.io/tx/${txHash}`);
  return txHash;
}
async function main() {
